...
/// Container for all the information necessary to build a StepperCustom widget's
/// forward and backward controls for any given step.
///
/// Used by [StepperCustom.controlsBuilder].
@immutable
class ControlsDetailsCustom {
  /// Creates a set of details describing the StepperCustom.
  const ControlsDetailsCustom({
  ...
  /// Index that is active for the surrounding [StepperCustom] widget. This may be
  ...
  /// just activated the transition to a new step, some [StepperCustom.type] values will
...
class StepCustom {
  ...
  /// Only [StepperCustomType.horizontal], Optional widget that appears under the [title].
  /// By default, uses the `bodyText1` theme.
  final Widget? label;
}

/// A material stepper widget that displays progress through a sequence of
/// steps. StepperCustoms are particularly useful in the case of forms where one step
...
///
/// {@tool dartpad}
/// An example the shows how to use the [StepperCustom], and the [StepperCustom] UI
/// appearance.
///
...
class StepperCustom extends StatefulWidget {
  ...
  const StepperCustom({
    required this.steps,
    this.physics,
    this.type = StepperCustomType.vertical,
    this.currentStep = 0,
    this.onStepTapped,
    this.onStepContinue,
    this.onStepCancel,
  ...
  /// [StepperCustomType.horizontal], the content of the current step is displayed
  /// underneath as opposed to the [StepperCustomType.vertical] case where it is
  /// displayed in-between.
  final StepperCustomType type;
  ...
  ///
  /// This callback which takes in a context and a [ControlsDetailsCustom] object, which
  ...
  /// [ControlsDetailsCustom.currentStep] value within the callback can change the text
  ...
  /// The elevation of this stepper's [Material] when [type] is [StepperCustomType.horizontal].
  final double? elevation;

  /// custom margin on vertical stepper.
  final EdgeInsetsGeometry? margin;

  /// custom padding on horizontal stepper.
  ...
  Widget _buildVerticalControls(int stepIndex) {
    ...
    return Container(
      ...
      child: ConstrainedBox(
        ...
        child: Row(
          // The Material spec no longer includes a StepperCustom widget. The continue
  ...
  Widget build(BuildContext context) {
    ...
    assert(() {
      if (context.findAncestorWidgetOfExactType<StepperCustom>() != null) {
        throw FlutterError(
          'StepperCustoms must not be nested.\n'
    ...
    switch (widget.type) {
      case StepperCustomType.vertical:
        return _buildVertical();
      case StepperCustomType.horizontal:
